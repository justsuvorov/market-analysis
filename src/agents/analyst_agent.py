    ...
    if _agent is None:
        _agent = Agent(
            ...
            system_prompt=(
                ...
                "ВСЕГДА пиши полные развёрнутые разделы — никогда не сокращай и не пропускай разделы. "
                "Используй данные из инструментов в каждом разделе."
            ),
        )

        @_agent.tool
        async def get_competitors_data(ctx: RunContext[RawResearchData]) -> str:
            """Вернуть краткую сводку по конкурентам для конкурентного анализа."""
            raw = ctx.deps.competitor_raw
            if not raw:
                return "Данные о конкурентах не собраны."
            # Компактный формат: только ключевые поля
            lines = ["Конкуренты в Ленобласти (топ-10):"]
            for c in raw[:10]:
                price = c.get("price_per_night_rub")
                price_str = f"{price:,} руб/ночь" if price else "цена н/д"
                rating = c.get("rating", "н/д")
                failed = " [ЗАКРЫЛСЯ/НЕУДАЧА]" if c.get("is_failed_case") else ""
                features = ", ".join(c.get("unique_features", [])[:3])
                lines.append(
                    f"- {c['name']} ({c.get('location', '?')}): {price_str}, "
                    f"рейтинг {rating}{failed}. УТП: {features or 'нет данных'}"
                )
            return "\n".join(lines)
...
async def run(raw: RawResearchData | None = None) -> tuple[str, str]:
    ...
    prompt = (
        "Ты должен написать ПОЛНЫЙ маркетинговый отчёт — минимум 8 разделов, "
        "каждый раздел минимум 3-5 абзацев с конкретными цифрами и выводами. "
        "Сначала вызови ВСЕ три инструмента: get_competitors_data, get_trends_data, get_location_scores. "
        "Затем напиши отчёт целиком в одном Markdown-ответе.\n\n"
        "# Маркетинговое исследование: Коттеджный комплекс, Ленобласть\n\n"
        "## 1. Обзор рынка загородной недвижимости Ленобласти\n"
        "(объём рынка, тренды, сезонность, ключевые драйверы роста)\n\n"
        "## 2. Конкурентный анализ\n"
        "(используй get_competitors_data: таблица конкурентов, ценовые сегменты, "
        "УТП лидеров, анализ провальных кейсов)\n\n"
        "## 4. Анализ целевой аудитории\n"
        "### 4.1 Инвесторы\n### 4.2 Покупатели домов\n### 4.3 Арендаторы\n"
        "(портрет, мотивации, боли, каналы привлечения)\n\n"
        "## 5. Концепции проекта (3 варианта с УТП и ценовым позиционированием)\n\n"
        "## 6. Вопросы для экспертных интервью (15-20 вопросов)\n\n"
        "## 7. SWOT-анализ\n\n"
        "## 8. Выводы и рекомендации\n\n"
        "ВАЖНО: напиши полный текст каждого раздела, не пропускай разделы, "
        "используй конкретные данные из инструментов. Весь текст на русском языке."
    )
